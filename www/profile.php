...
<body>
    ...
    $id_uz =  mysqli_real_escape_string($mysqli, $_SESSION["userID"]);
    ...
    <div id="formContainer">
        <form action="" method="post"> 
            ...
            <div id="info">
                ...
                $sql = "UPDATE klienci SET imie=?,nazwisko=?,nr_tel=?,email=?,data_ur=? WHERE id=?"; 
                $stmt = $mysqli->prepare($sql);
                $stmt->bind_param("ssissi", $firstName, $lastName, $phoneNumber, $email, $birthDate, $id_k);
                $stmt->execute();

                $sql = "UPDATE uzytkownicy SET email=? WHERE id=?"; 
                $stmt = $mysqli->prepare($sql);
                $stmt->bind_param("si", $email, $id_uz);
                $stmt->execute();
                if(isEmployee()){
